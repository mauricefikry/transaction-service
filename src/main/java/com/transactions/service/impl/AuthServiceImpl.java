...
public class AuthServiceImpl implements AuthService {
  ...
  public ResponseEntity<ApiResponse<LoginResponse>> signIn(LoginRequest request) {
    ...
    Optional<User> user =
        userRepo.findByUsernameOrEmail(
            request.getUsernameOrEmail().toUpperCase(), request.getUsernameOrEmail().toUpperCase());

    if (user.isEmpty()) {
      return new ResponseEntity<>(
          new ApiResponse<>(false, "User not exist", 400, null), HttpStatus.BAD_REQUEST);
    }

    boolean exist =
        passwordEncoder.matches(
            request.getPassword().replaceAll("\\s", ""), user.get().getPassword());
    if (!exist) {
      return new ResponseEntity<>(
          new ApiResponse<>(false, "Password Wrong", 400, null), HttpStatus.BAD_REQUEST);
    }

    if (Boolean.TRUE.equals(user.get().getIsDeleted())) {
      return new ResponseEntity<>(
          new ApiResponse<>(false, "User Not Found, User deleted", 400, null),
    ...
    return ResponseEntity.ok(
        new ApiResponse<>(true, "User Sign In successfully", 0, loginResponse));
  }
  ...
  public ResponseEntity<ApiResponse<SignUpResponse>> signUp(SignUpRequest request) {
    ...
    if (userRepo.existsByEmail(request.getEmail().toUpperCase())) {
      return new ResponseEntity<>(
          new ApiResponse<>(false, "Email is already in use", 400, null), HttpStatus.BAD_REQUEST);
    }
    ...
    return ResponseEntity.ok(
        new ApiResponse<>(true, "User registered successfully", 0, signUpResponse));
  }
}
