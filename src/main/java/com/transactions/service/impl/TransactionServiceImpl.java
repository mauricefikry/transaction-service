...
public class TransactionServiceImpl implements TransactionService {
    ...
    public ResponseEntity<ApiResponse<TransactionResponse>> getTransactionById(UUID id) {
        ...
        if (transaction.isEmpty()) {
            return new ResponseEntity<>(
                    new ApiResponse<>(
                            false,
                            "Get Transaction By Id Failed, Id Not Found",
                            404,
                            null
    ...
    public ResponseEntity<ApiResponse<TransactionResponse>> createTransaction(
        ...
        if (user.isEmpty()) {
            return new ResponseEntity<>(
                    new ApiResponse<>(
                            false,
                            "Create Transaction Failed, User Not Found",
                            404,
                            null
                    ), HttpStatus.BAD_REQUEST
            );
        ...
        if (productOptional.isEmpty()) {
            return new ResponseEntity<>(
                    new ApiResponse<>(
                            false,
                            "Create Transaction Failed, Product Not Found",
                            404,
                            null
                    ), HttpStatus.BAD_REQUEST
            );
    ...
    public ResponseEntity<ApiResponse<TransactionResponse>> updateTransaction(
        ...
        if (transaction.isEmpty()) {
            return new ResponseEntity<>(
                    new ApiResponse<>(
                            false,
                            "Delete Transaction By Id Failed, Id Not Found",
                            404,
                            null
                    ), HttpStatus.BAD_REQUEST
            );
        ...
        if (null != request.getNetAmountPaid()) transaction.get().setNetAmountPaid(request.getNetAmountPaid());
        if (null != request.getTotalAmountPaid()) transaction.get().setNetAmountPaid(request.getTotalAmountPaid());
        if (null != request.getTotalTaxPaid()) transaction.get().setNetAmountPaid(request.getTotalTaxPaid());
        if (null != request.getTransactionTime()) transaction.get().setTransactionTime(request.getTransactionTime());
        if (null != request.getPaymentStatus()) transaction.get().setPaymentStatus(request.getPaymentStatus());
        if (null != request.getPaymentMethod()) transaction.get().setPaymentMethod(request.getPaymentMethod());
    ...
    public ResponseEntity<ApiResponse<TransactionResponse>> deleteTransaction(
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(
                        true,
                        "Delete Transaction By Id successfully",
                        0,
                        null
                )
        );
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getReportSummary(
        ...
        if (start != null && end != null) {
            BigDecimal totalByCustomerBetween = transactionRepo.totalAmountBetweenTwoDates(start, end, userId);
            reportSummary.put("totalByCustomerBetweenDates", totalByCustomerBetween != null ? totalByCustomerBetween : BigDecimal.ZERO);
        }

        BigDecimal totalByCustomer = transactionRepo.total(userId);
        reportSummary.put("totalTransaction", totalByCustomer != null ? totalByCustomer : BigDecimal.ZERO);
        ...
        return ResponseEntity.ok(
                new ApiResponse<>(
                        true,
                        "Get Report Summary Transaction successfully",
                        0,
                        reportSummary
                )
        );
