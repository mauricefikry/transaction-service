...
public interface TransactionRepo extends JpaRepository<Transaction, UUID>,
    ...
    @Query("SELECT SUM(t.totalAmountPaid) " +
            "FROM Transaction t")
    BigDecimal total(@Param("userId") UUID userId);

    @Query("SELECT t.paymentMethod, SUM(t.totalTaxPaid) FROM Transaction t " +
            "WHERE t.user.id = :userId GROUP BY t.paymentMethod")
    List<Object[]> totalPerTax(@Param("userId") UUID userId);

    @Query("SELECT t.product.name, SUM(t.totalAmountPaid) FROM Transaction t " +
